...
pickup_col = next((c for c in ["tpep_pickup_datetime","lpep_pickup_datetime","pickup_datetime","pickup_time"] if c in df.columns), None)
dropoff_col = next((c for c in ["tpep_dropoff_datetime","lpep_dropoff_datetime","dropoff_datetime","dropoff_time"] if c in df.columns), None)

if pickup_col is None or dropoff_col is None:
    # If timestamps aren’t there, we expect duration_min already exists.
    if "duration_min" not in df.columns:
        raise ValueError("Could not find pickup/dropoff timestamps and no 'duration_min' present. "
...
num_cols = [c for c in ["trip_distance","fare_amount","passenger_count"] if c in df.columns]

# Location IDs (categorical)
cat_cols = [c for c in ["PULocationID","DOLocationID","payment_type","RatecodeID","store_and_fwd_flag"] if c in df.columns]
...
if not num_cols and not cat_cols:
    ...
    guess_num = [c for c in df.columns if pd.api.types.is_numeric_dtype(df[c]) and c != "duration_min"]
    if not guess_num:
